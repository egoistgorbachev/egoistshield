...
import { buildNode, buildVlessUri, buildVmessUriFromMetadata, toNumberSafe, toStringSafe } from "./parser-utils";
import { parseNodeUriDetailed } from "./uri-parsers";

export function parseXrayOutbound(
  outbound: Record<string, unknown>,
  index: number
): { node: VpnNode | null; issue: string | null } {
  ...
  const settings =
    outbound.settings && typeof outbound.settings === "object" ? (outbound.settings as Record<string, unknown>) : {};
  const streamSettings =
    outbound.streamSettings && typeof outbound.streamSettings === "object"
  ...
  if (protocol === "vless") {
    ...
    const port = toNumberSafe(record.port, Number.NaN);
    ...
    const tlsSettings =
      streamSettings.tlsSettings && typeof streamSettings.tlsSettings === "object"
        ? (streamSettings.tlsSettings as Record<string, unknown>)
        : {};
    const realitySettings =
      streamSettings.realitySettings && typeof streamSettings.realitySettings === "object"
        ? (streamSettings.realitySettings as Record<string, unknown>)
        : {};
    const wsSettings =
      streamSettings.wsSettings && typeof streamSettings.wsSettings === "object"
    ...
    const headers =
      wsSettings.headers && typeof wsSettings.headers === "object"
...
export function parseSingBoxOutbound(
  ...
  const port = toNumberSafe(outbound.server_port, Number.NaN);
  ...
  const collectFromObject = (obj: Record<string, unknown>) => {
    ...
    if (Array.isArray(obj.outbounds)) {
      for (let i = 0; i < obj.outbounds.length; i += 1) {
        ...
        const parsedOutbound = hasXrayProtocol ? parseXrayOutbound(asRecord, i) : parseSingBoxOutbound(asRecord, i);
