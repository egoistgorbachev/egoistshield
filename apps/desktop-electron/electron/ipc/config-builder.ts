import { parseCustomDnsUrl } from "../../shared/secure-dns";
import type {
	AppSettings,
	DomainRule,
	ProcessRule,
...
export namespace ConfigBuilder {
	export function buildXray(
		...
		const config: XrayConfig = {
			...
			routing: {
				domainStrategy:
					settings.dnsMode === "secure" || settings.dnsMode === "custom"
						? "IPOnDemand"
						: "AsIs",
				rules: [
	...
	export function buildSingBox(
		...
		const userRules = buildSingBoxRules(
			domainRules,
			processRules,
			settings.useTunMode,
		);
		...
		const config: SingBoxConfig = {
			...
			dns: buildSingBoxDns(settings.dnsMode, settings.customDnsUrl),
			inbounds: [
				{
					type: "mixed",
					tag: "mixed-in",
		...
		if (node.protocol === "vless") {
			return {
							...
							users: [
								{
									...
									flow:
										m.flow ||
										(m.security === "reality" ? "xtls-rprx-vision" : undefined),
								},
							],
						},
					],
				...
				streamSettings: {
					...
					httpSettings:
						m.type === "h2"
							? { path: m.path ?? "/", host: m.host ? [m.host] : undefined }
							: undefined,
				},
			};
		}
				...
				streamSettings: {
					...
					wsSettings:
						m.net === "ws"
							...
							: undefined,
					grpcSettings:
						m.net === "grpc" ? { serviceName: m.serviceName ?? "" } : undefined,
				},
			};
		}
	...
	function buildSingBoxOutbound(node: VpnNode): Record<string, unknown> {
		...
		if (node.protocol === "vless") {
			...
			return {
				...
				uuid: m.id ?? "",
				flow:
					m.flow || (m.security === "reality" ? "xtls-rprx-vision" : undefined),
				tls: tls,
				transport: transport,
			};
		}
		...
		if (node.protocol === "tuic") {
			return {
				...
				password: m.password ?? "",
				congestion_control:
					m.congestion_control ?? profile.tuicCongestionControl ?? "bbr",
				udp_relay_mode:
					m.udp_relay_mode ?? profile.tuicUdpRelayMode ?? "native",
				zero_rtt_handshake:
					parseBool(m.zero_rtt_handshake) ||
					profile.tuicZeroRttHandshake === true,
			...
			for (const rule of processRules) {
				...
				if (rule.mode === "block") {
					...
					rules.push({
						...
						outbound: rule.mode === "vpn" ? "proxy" : "direct",
					});
				}
			}
		}
	...
	function buildDefaultXrayRule(
		mode: AppSettings["routeMode"],
	): Record<string, unknown> {
		return {
			type: "field",
			network: "tcp,udp",
	...
	function buildDns(
		mode: AppSettings["dnsMode"],
		customDnsUrl?: string,
	): Record<string, unknown> | undefined {
		if (mode === "custom") {
			return {
				servers: [parseCustomDnsUrl(customDnsUrl ?? "").url],
			};
		}
				...
				servers: [
					{
						...
						domain_resolver: customDns.hostnameRequiresResolver
							? { server: "bootstrap-dns", strategy: "prefer_ipv4" }
							: undefined,
					},
