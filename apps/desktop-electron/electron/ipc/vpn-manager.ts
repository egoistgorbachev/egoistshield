...
export class VpnRuntimeManager extends EventEmitter {
  ...
  public async connect(
    ...
    this.snapshot.processRulesApplied =
      processRules.length > 0 && resolvedRuntime.runtimeKind === "sing-box" && effectiveSettings.useTunMode;
    ...
    if (!ready && child.exitCode !== null) {
      ...
      if (SINGBOX_PROTOCOLS.has(node.protocol)) {
        ...
        this.snapshot.lastError = `Протокол ${node.protocol} требует sing-box, но runtime крашнулся при старте.${runtimeLog ? ` Лог: ${runtimeLog}` : ""}${
