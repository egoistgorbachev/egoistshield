...
export function normalizeZapretProfile(
	value: unknown,
	fallback: string,
): string {
	if (!isString(value)) {
...
function sanitizePersistedState(
	...
	return {
		...currentState,
		fakeDns: isBoolean(persisted.fakeDns)
			? persisted.fakeDns
			: currentState.fakeDns,
		killSwitch: isBoolean(persisted.killSwitch)
			? persisted.killSwitch
			: currentState.killSwitch,
		autoUpdate: isBoolean(persisted.autoUpdate)
			? persisted.autoUpdate
			: currentState.autoUpdate,
		autoConnect: isBoolean(persisted.autoConnect)
			? persisted.autoConnect
			: currentState.autoConnect,
		notifications: isBoolean(persisted.notifications)
			? persisted.notifications
			: currentState.notifications,
		autoStart: isBoolean(persisted.autoStart)
			? persisted.autoStart
			: currentState.autoStart,
		systemDnsServers: isString(persisted.systemDnsServers)
			? persisted.systemDnsServers
			: currentState.systemDnsServers,
		customDnsUrl: normalizeCustomDnsUrl(
			persisted.customDnsUrl,
			currentState.customDnsUrl,
		),
		systemDohEnabled: isBoolean(persisted.systemDohEnabled)
			? persisted.systemDohEnabled
			: currentState.systemDohEnabled,
		systemDohUrl: normalizeSystemDohUrl(
			persisted.systemDohUrl,
			currentState.systemDohUrl,
		),
		systemDohLocalAddress: normalizeSystemDohLocalAddress(
			persisted.systemDohLocalAddress,
			currentState.systemDohLocalAddress,
		),
		zapretProfile: normalizeZapretProfile(
			persisted.zapretProfile,
			currentState.zapretProfile,
		),
		...
		selectedServerId: isString(persisted.selectedServerId)
			? persisted.selectedServerId
			: currentState.selectedServerId,
		servers: Array.isArray(persisted.servers)
			? (persisted.servers as AppState["servers"])
	...
	persist(
		...
		}),
		{
			...
			merge: (persistedState, currentState) =>
				sanitizePersistedState(persistedState, currentState),
			partialize: (state) => ({
				...
				systemDohEnabled: state.systemDohEnabled,
				systemDohUrl: state.systemDohUrl,
				systemDohLocalAddress: state.systemDohLocalAddress,
