...
function nextProbeConfidence(previous: number | null | undefined, outcome: "success" | "failure" | "boost"): number {
  const baseConfidence = clamp(normalizeScore(previous, DEFAULT_PROBE_CONFIDENCE), 0.05, 1);
  if (outcome === "boost") {
...
function computeQualityScore(metrics: {
  ...
  return Math.round(
    clamp(100 - latencyPenalty - jitterPenalty - lossPenalty - connectPenalty - handshakePenalty, 0, 100)
  );
}
...
function computeStabilityScore(
  ...
  const previousScore = clamp(normalizeScore(server.stabilityScore, DEFAULT_STABILITY_SCORE), 0, 100);
  const normalizedSuccessRate = normalizeSuccessRate(successRate);
  const degradationPenalty = Math.min(18, normalizeScore(degradationCount, 0) * 4);
  const targetScore = clamp(qualityScore * 0.62 + normalizedSuccessRate * 38 - degradationPenalty, 0, 100);
  return Math.round(previousScore * 0.45 + targetScore * 0.55);
}

function reduceCooldownAfterSuccessfulProbe(cooldownUntil: number | null | undefined, now: number): number | null {
...
function getFallbackSourceRuntime(status: RuntimeStatus): RuntimeKind | null {
  if (!status.diagnostic.fallbackAttempted || !status.diagnostic.fallbackTarget) {
    return null;
  }
...
function clearRecoveredRuntimePenalty(
  ...
  if (typeof server.runtimePenaltyUntil === "number" && server.runtimePenaltyUntil <= now) {
    return {
      runtimePenaltyKind: null,
      runtimePenaltyUntil: null
    };
...
function restoreServersForQualityContext(
  ...
  return servers.map((server) => restoreQualityFromCache(server, qualityContextKey, now));
}
...
function persistServerQualitySnapshots(
  ...
  return servers.map((server) =>
    touchedIds.has(server.id) ? upsertQualityCache(server, qualityContextKey, now) : server
  );
}
...
function getRotatedServers(servers: PingableServer[], offset: number): PingableServer[] {
  if (servers.length === 0) {
    return [];
  }

  const normalizedOffset = ((offset % servers.length) + servers.length) % servers.length;
  return [...servers.slice(normalizedOffset), ...servers.slice(0, normalizedOffset)];
}
...
export function selectServersForPingSweep(
  ...
  const remainingServers = servers.filter((server) => server.id !== activeServerId);
  ...
  if (options.connectionMode === "smart" && options.isConnected) {
    ...
    return {
      targets: dedupePingTargets(activeServer ? [activeServer, ...probeTargets] : probeTargets),
      nextOffset: options.sweepOffset
    };
  }

  const sampleSize = options.isConnected ? DEFAULT_CONNECTED_BACKGROUND_SAMPLE : DEFAULT_IDLE_BACKGROUND_SAMPLE;
  const rotatedServers = getRotatedServers(remainingServers, options.sweepOffset);
  const sampledServers = rotatedServers.slice(0, Math.min(sampleSize, rotatedServers.length));

  return {
    targets: dedupePingTargets(activeServer ? [activeServer, ...sampledServers] : sampledServers),
    nextOffset:
      remainingServers.length > 0
        ? (options.sweepOffset + Math.max(1, sampledServers.length)) % remainingServers.length
        : 0
  };
}
...
function createMeasuredCandidate(server: ServerConfig | undefined, result: PingProbeResult): SmartCandidateHealth {
  return {
  ...
  return servers.map((server) => {
    ...
    return {
      ...
      stabilityScore: probeSucceeded
        ? computeStabilityScore(server, qualityScore, server.successRate, server.degradationCount)
        : Math.max(0, computeStabilityScore(server, qualityScore, server.successRate, server.degradationCount) - 10),
      probeConfidence: nextProbeConfidence(server.probeConfidence, probeSucceeded ? "success" : "failure"),
      ...
      recentQualitySamples: appendRecentSample(server.recentQualitySamples ?? null, qualityScore),
      cooldownUntil: probeSucceeded
        ? reduceCooldownAfterSuccessfulProbe(server.cooldownUntil, result.checkedAt)
        : (server.cooldownUntil ?? null)
    };
  });
}

function getExistingServerHealth(server: ServerConfig | undefined): Partial<ServerConfig> {
  if (!server) {
    return {};
  }
...
function shouldRegisterFailure(server: ServerConfig, reason: RuntimeFailureReason | null, now: number): boolean {
  if (!reason) {
    return false;
  }
  ...
  return typeof server.lastFailureAt !== "number" || now - server.lastFailureAt > 45_000;
}
...
function updateFailureState(
  ...
  const nextFailureCount = registeredFailure ? (server.failureCount ?? 0) + 1 : (server.failureCount ?? 0);
  const qualityScore = computeQualityScore({
  ...
  const nextDegradationCount = (server.degradationCount ?? 0) + degradationIncrement + 1;
  const applyRuntimePenalty = shouldPenalizeRuntime(failureReason) && runtimeKind !== null && runtimeKind !== undefined;
  const runtimePenaltyUntil = applyRuntimePenalty
    ? computeCooldownUntil(Math.max(1, nextFailureCount), failureReason, server.protocol, now)
  ...
  return {
    ...
    cooldownUntil: computeCooldownUntil(Math.max(1, nextFailureCount), failureReason, server.protocol, now),
    stabilityScore: Math.max(
      0,
      computeStabilityScore(server, Math.max(0, qualityScore - 18), nextSuccessRate, nextDegradationCount) - 6
    ),
    probeConfidence: nextProbeConfidence(server.probeConfidence, "failure"),
    recentQualitySamples: appendRecentSample(server.recentQualitySamples ?? null, Math.max(0, qualityScore - 24)),
    runtimePenaltyKind: applyRuntimePenalty ? runtimeKind : (server.runtimePenaltyKind ?? null),
    runtimePenaltyUntil,
    ...
    if (status.connected && status.activeNodeId === serverId) {
      ...
      const runtimePenalty =
        fallbackSourceRuntime !== null
          ...
          : clearRecoveredRuntimePenalty(server, status.runtimeKind ?? null, now);
      return {
        ...server,
        connectTimeMs,
        timeToFirstByteMs: connectTimeMs,
        ...
        preferredRuntimeKind:
          status.diagnostic.fallbackTarget ?? status.runtimeKind ?? server.preferredRuntimeKind ?? null,
        ...
        cooldownUntil:
          status.lifecycle === "active" && !status.diagnostic.reason ? null : (server.cooldownUntil ?? null),
        lastFailureReason:
          status.lifecycle === "active" && !status.diagnostic.reason ? null : (server.lastFailureReason ?? null),
        lastFailureAt:
          status.lifecycle === "active" && !status.diagnostic.reason ? null : (server.lastFailureAt ?? null),
        ...runtimePenalty
      };
    }
...
function applyRuntimeHealthUpdate(
  ...
  return servers.map((server) => {
    ...
    const nextBase: ServerConfig = {
      ...
      preferredRuntimeKind: status.runtimeKind ?? server.preferredRuntimeKind ?? null,
      recentPingSamples: diagnosis.runtimeReachable
        ? appendRecentSample(server.recentPingSamples ?? null, diagnosis.latencyMs)
        : (server.recentPingSamples ?? null)
    };
    const qualityScore = computeQualityScore({
      ping: diagnosis.runtimeReachable ? diagnosis.latencyMs : Math.max(server.ping, 220),
      jitterMs: diagnosis.jitterMs,
      lossPercent: diagnosis.lossPercent,
      connectTimeMs: server.connectTimeMs,
      timeToFirstByteMs: server.timeToFirstByteMs
    ...
    const failureReason = status.diagnostic.reason ?? diagnosis.failureReason ?? server.lastFailureReason ?? "unknown";
    return updateFailureState(
      nextBase,
      failureReason,
      now,
...
function shouldAttemptDegradedSwitch(status: RuntimeStatus, diagnosis: DiagnosticResult): boolean {
...
export interface ServersSlice {
  ...
  recordRuntimeHealth: (status: RuntimeStatus, diagnosis: DiagnosticResult) => void;
}
...
> = (set, get) => ({
  ...
  connectToServer: async (id) => {
    ...
    set({ selectedServerId: id, isConnecting: true, isDisconnecting: wasConnected, errorMessage: null });
    ...
    try {
      ...
      const nextServers = persistServerQualitySnapshots(
        applyConnectionOutcome(get().servers, id, status, connectTimeMs, finishedAt),
        qualityContextKey,
  ...
  togglePinServer: async (id) => {
    set((state) => ({
      servers: state.servers.map((server) => (server.id === id ? { ...server, pinned: !server.pinned } : server))
    }));
    ...
    const updatedNodes = currentState.nodes.map((node) => {
      if (node.id === id) {
        return {
          ...node,
          metadata: { ...node.metadata, pinned: node.metadata?.pinned === "true" ? "false" : "true" }
        };
      }
  ...
  removeSubscription: async (url) => {
    ...
    const currentState = await api.state.get();
    const subscriptionToRemove = currentState.subscriptions.find((subscription) => subscription.url === url);
    const subscriptionId = subscriptionToRemove?.id;
    const filteredNodes = subscriptionId
      ? currentState.nodes.filter((node) => node.subscriptionId !== subscriptionId)
      : currentState.nodes;

    await api.state.set({
      ...currentState,
      subscriptions: currentState.subscriptions.filter((subscription) => subscription.url !== url),
      nodes: filteredNodes,
      activeNodeId: filteredNodes.some((node) => node.id === currentState.activeNodeId)
  ...
  renameSubscription: async (url, newName) => {
    set((state) => ({
      subscriptions: state.subscriptions.map((subscription) =>
        subscription.url === url ? { ...subscription, name: newName } : subscription
      )
    }));
  ...
  renameServer: async (id, newName) => {
    set((state) => ({
      servers: state.servers.map((server) => (server.id === id ? { ...server, name: newName } : server))
    }));
  ...
  syncWithBackend: async () => {
    ...
    const previousServers = new Map(get().servers.map((server) => [server.id, server]));
    const mappedServers: ServerConfig[] = state.nodes.map((node) => {
      ...
      return {
        ...
        security: node.metadata?.security || (node.metadata?.flow ? "reality" : ""),
        premium: node.metadata?.premium === "true" || !!node.name?.toLowerCase().match(/premium|vip|pro|plus/i),
        _host: node.server,
        _port: node.port
      };
    });
    ...
    const hydratedServers = restoreServersForQualityContext(mappedServers, qualityContextKey);

    const currentUiSelection = get().selectedServerId;
    const uiSelectionValid = currentUiSelection && hydratedServers.some((server) => server.id === currentUiSelection);

    set({
      servers: hydratedServers,
      subscriptions: state.subscriptions || [],
      selectedServerId: uiSelectionValid ? currentUiSelection : state.activeNodeId || hydratedServers[0]?.id || "",
      fakeDns: state.settings.dnsMode === "secure",
      killSwitch: state.settings.killSwitch,
    ...
    (async () => {
      ...
      const unknownServers = get().servers.filter(
        (server): server is GeoIpServer => server.countryCode === "un" && isGeoIpServer(server)
      );
      for (let index = 0; index < unknownServers.length; index += 5) {
        ...
        const results = await Promise.allSettled(
          chunk.map(async (server) => {
            ...
            return { id: server.id, countryCode: geo.countryCode, country: geo.country };
          })
        );

        const updates = new Map<string, { countryCode: string; country: string }>();
        for (const result of results) {
          if (result.status === "fulfilled" && result.value.countryCode && result.value.countryCode !== "un") {
            updates.set(result.value.id, { countryCode: result.value.countryCode, country: result.value.country });
          }
        }
        ...
        if (updates.size > 0) {
          set({
            servers: get().servers.map((server) => {
              ...
              return update ? { ...server, countryCode: update.countryCode, countryName: update.country } : server;
            })
          });
        }
      }
  ...
  startPingLoop: () => {
    ...
    const interval = setInterval(() => {
      ...
      const state = get();
      void state.testAllPings(state.connectionMode === "smart" && state.isConnected);
    }, 2_000);
  ...
  testAllPings: async (activeOnly?: boolean) => {
    ...
    const servers = restoreServersForQualityContext(state.servers, qualityContextKey).filter(
      (server): server is PingableServer => server.id !== "smart-optimal" && isPingableServer(server)
    );
    if (servers.length === 0) {
      return;
    }

    const activeServer = servers.find((server) => server.id === (state.connectedServerId || state.selectedServerId));
    const smartProbeBudget = computeAdaptiveProbeBudget({
      serverCount: servers.length,
      stage:
        state.connectionMode === "smart" && state.isConnected
          ? state._runtimeLifecycle === "degraded" || state._runtimeFailureReason !== null
            ? "degraded"
            : "monitor"
          : state.isConnected
            ? "default_connected"
  ...
  smartConnect: async () => {
    ...
    const servers = restoreServersForQualityContext(state.servers, qualityContextKey).filter(isPingableServer);
    if (servers.length === 0) {
      return;
    }
    ...
    const fallbackCandidates = attemptOrder.length > 0 ? attemptOrder : initialPlan.fallbackCandidates;
  ...
  evaluateSmartSwitch: async (trigger = "timer") => {
    ...
    const now = Date.now();
    const cachedCandidates = servers.map(toSmartCandidate);
    const connectedServer = servers.find((server) => server.id === state.connectedServerId);
    ...
    const mergedCandidates = mergeSmartCandidates(cachedCandidates, measuredCandidates, now);
    const switchPlan = planAutoSwitch(mergedCandidates, state.connectedServerId, state._smartSwitchTracker, {
      now,
      currentLifecycle: state._runtimeLifecycle
    });
    ...
    if (trigger === "timer" && state._runtimeLifecycle === "active" && state._runtimeFailureReason === null) {
      const connectedServer = state.servers.find((server) => server.id === state.connectedServerId);
      const hasCurrentDegradation =
        (connectedServer?.lossPercent ?? 0) >= SMART_HEALTH_DEGRADATION_LOSS_PERCENT ||
    ...
    if (afterSwitchState.isConnected && afterSwitchState.connectedServerId === switchPlan.recommendedCandidate.id) {
      set({
        _smartSwitchTracker: {
          candidateId: null,
          consecutiveWins: 0,
  ...
  recordRuntimeHealth: (status, diagnosis) => {
    ...
    set((currentState) => ({
      ...
      errorMessage: status.lifecycle === "degraded" && status.lastError ? status.lastError : currentState.errorMessage,
      _runtimeLifecycle: status.lifecycle,
      _runtimeFailureReason: status.diagnostic.reason ?? diagnosis.failureReason ?? null,
      servers: status.activeNodeId
        ? persistServerQualitySnapshots(
            applyRuntimeHealthUpdate(currentState.servers, status, diagnosis, now),
            qualityContextKey,
            [status.activeNodeId],
            now
          )
    ...
    if (get().connectionMode === "smart" && status.activeNodeId && shouldAttemptDegradedSwitch(status, diagnosis)) {
