import { createSecureDnsContextProfile } from "../../../shared/secure-dns";
...
function snapshotServerQuality(
	server: QualityCachedServerLike,
	contextKey: string,
	now: number,
): QualityCacheEntry {
	return {
...
function decayMetric(
	...
	const decayRange = Math.max(
		1,
		QUALITY_CACHE_DECAY_FULL_MS - QUALITY_CACHE_DECAY_START_MS,
	);
...
export function createQualityContextKey(options: {
	...
	const dnsProfile =
		options.systemDnsServers.trim().length > 0
			? "system-custom"
			: "system-default";
	const secureDnsProfile = createSecureDnsContextProfile({
		fakeDns: options.fakeDns,
		customDnsUrl: options.customDnsUrl,
	});
	const killSwitchProfile = options.killSwitch
		? "kill-switch-on"
		: "kill-switch-off";
...
export function upsertQualityCache<T extends QualityCachedServerLike>(
	...
	const remainingEntries = (server.qualityCache ?? []).filter(
...
export function restoreQualityFromCache<T extends QualityCachedServerLike>(
	...
	const cacheEntry = (server.qualityCache ?? []).find(
		(entry) => entry.contextKey === contextKey,
	);
	if (!cacheEntry) {
		return server;
	...
	const shouldHydratePing =
		cacheLastPingAt > baseLastPingAt || baseLastPingAt === 0;

	return {
		...server,
		ping:
			shouldHydratePing && typeof cacheEntry.ping === "number"
				? cacheEntry.ping
				: server.ping,
		lastPingAt: shouldHydratePing
			? (cacheEntry.lastPingAt ?? server.lastPingAt ?? null)
			: (server.lastPingAt ?? null),
		...
		timeToFirstByteMs:
			cacheEntry.timeToFirstByteMs ?? server.timeToFirstByteMs ?? null,
		...
		lastFailureReason:
			cacheEntry.lastFailureReason ?? server.lastFailureReason ?? null,
		cooldownUntil: cacheEntry.cooldownUntil ?? server.cooldownUntil ?? null,
		preferredRuntimeKind:
			cacheEntry.preferredRuntimeKind ?? server.preferredRuntimeKind ?? null,
		lastSuccessfulAt:
			cacheEntry.lastSuccessfulAt ?? server.lastSuccessfulAt ?? null,
		stabilityScore:
			decayMetric(cacheEntry.stabilityScore, ageMs, 0.65) ??
			server.stabilityScore ??
			null,
		probeConfidence:
			decayMetric(cacheEntry.probeConfidence, ageMs, 0.55) ??
			server.probeConfidence ??
			null,
		runtimePenaltyUntil:
			cacheEntry.runtimePenaltyUntil ?? server.runtimePenaltyUntil ?? null,
		runtimePenaltyKind:
			cacheEntry.runtimePenaltyKind ?? server.runtimePenaltyKind ?? null,
		recentPingSamples:
			sanitizeHistory(cacheEntry.recentPingSamples) ??
			sanitizeHistory(server.recentPingSamples),
		recentQualitySamples:
			sanitizeHistory(cacheEntry.recentQualitySamples) ??
			sanitizeHistory(server.recentQualitySamples),
		degradationCount:
			decayMetric(cacheEntry.degradationCount, ageMs, 0.4) ??
