...
export function getPingStyle(ping: number | null): { color: string; glow: string; text: string } {
    if (!ping || ping <= 0) return { color: "text-white/30", glow: "", text: "--" };
    if (ping < 80) return { color: "text-emerald-400", glow: "shadow-[0_0_8px_rgba(52,211,153,0.4)]", text: `${ping} мс` };
    if (ping < 200) return { color: "text-yellow-400", glow: "shadow-[0_0_8px_rgba(250,204,21,0.4)]", text: `${ping} мс` };
