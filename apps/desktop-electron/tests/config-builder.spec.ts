...
const parseXrayConfig = (config: string): XrayConfig => JSON.parse(config) as XrayConfig;
const parseSingBoxConfig = (config: string): SingBoxConfig => JSON.parse(config) as SingBoxConfig;
...
describe("config-builder", () => {
  ...
  it("включает process rules в sing-box при TUN", () => {
    ...
    const processRule = config.route.rules.find((rule) => Array.isArray(rule.process_name));
    const domainRule = config.route.rules.find((rule) => Array.isArray(rule.domain_suffix));
  ...
  it("использует DNS через прокси в sing-box при dnsMode=auto (v1.12 формат)", () => {
    const config = parseSingBoxConfig(
      ConfigBuilder.buildSingBox(hy2Node, [], [], { ...baseSettings, dnsMode: "auto" }, SING_BOX_MIXED_PORT)
    );
    const proxyDns = config.dns.servers.find((server) => server.tag === "proxy-dns");
  ...
  it("использует route.final для sing-box global/selected", () => {
    const globalConfig = parseSingBoxConfig(
      ConfigBuilder.buildSingBox(hy2Node, [], [], { ...baseSettings, routeMode: "global" }, SING_BOX_MIXED_PORT)
    );
    const selectedConfig = parseSingBoxConfig(
      ConfigBuilder.buildSingBox(hy2Node, [], [], { ...baseSettings, routeMode: "selected" }, SING_BOX_MIXED_PORT)
    );
  ...
  it("sing-box v1.12: sniff и hijack-dns через rule actions", () => {
    const config = parseSingBoxConfig(
      ConfigBuilder.buildSingBox(vlessNode, [], [], { ...baseSettings, routeMode: "global" }, SING_BOX_MIXED_PORT)
    );

    const mixed = config.inbounds.find((inbound) => inbound.type === "mixed");
    const sniffRule = config.route.rules.find((rule) => rule.action === "sniff");
    const hijackRule = config.route.rules.find((rule) => rule.action === "hijack-dns");

    expect(mixed?.sniff).toBeUndefined();
    expect(sniffRule).toBeDefined();
    expect(hijackRule?.protocol).toBe("dns");
    expect(config.outbounds.find((outbound) => outbound.type === "block")).toBeUndefined();
    expect(config.outbounds.find((outbound) => outbound.type === "dns")).toBeUndefined();
  });
  ...
  it("sing-box v1.12: block rules используют action: reject", () => {
    ...
    const blockRule = config.route.rules.find((rule) => rule.domain_suffix?.[0] === "ads.com");
    const vpnRule = config.route.rules.find((rule) => rule.domain_suffix?.[0] === "vpn.com");
  ...
  it("Trojan Xray: корректный outbound с паролем", () => {
    ...
    const config = parseXrayConfig(
      ConfigBuilder.buildXray(trojanNode, [], baseSettings, XRAY_HTTP_PORT, XRAY_SOCKS_PORT, XRAY_API_PORT)
    );

    const outbound = config.outbounds.find((outboundItem) => outboundItem.protocol === "trojan");
  ...
  it("VMess Xray: корректный outbound с alterId и security", () => {
    ...
    const config = parseXrayConfig(
      ConfigBuilder.buildXray(vmessNode, [], baseSettings, XRAY_HTTP_PORT, XRAY_SOCKS_PORT, XRAY_API_PORT)
    );

    const outbound = config.outbounds.find((outboundItem) => outboundItem.protocol === "vmess");
  ...
  it("Shadowsocks Xray: method и password", () => {
    ...
    const config = parseXrayConfig(
      ConfigBuilder.buildXray(ssNode, [], baseSettings, XRAY_HTTP_PORT, XRAY_SOCKS_PORT, XRAY_API_PORT)
    );

    const outbound = config.outbounds.find((outboundItem) => outboundItem.protocol === "shadowsocks");
  ...
  it("пустой metadata → fallback значения", () => {
    ...
    const config = parseXrayConfig(
      ConfigBuilder.buildXray(emptyNode, [], baseSettings, XRAY_HTTP_PORT, XRAY_SOCKS_PORT, XRAY_API_PORT)
    );

    const outbound = config.outbounds.find((outboundItem) => outboundItem.protocol === "vless");
  ...
  it("sing-box TUN без process rules → нет process_name в правилах", () => {
    const config = parseSingBoxConfig(
      ConfigBuilder.buildSingBox(hy2Node, [], [], { ...baseSettings, useTunMode: true }, SING_BOX_MIXED_PORT)
    );
  ...
  it("Shadowsocks sing-box: outbound корректен", () => {
    ...
    const config = parseSingBoxConfig(ConfigBuilder.buildSingBox(ssNode, [], [], baseSettings, SING_BOX_MIXED_PORT));
  ...
  it("Hysteria2 sing-box: профиль фиксирует UDP и hop interval", () => {
    const config = parseSingBoxConfig(ConfigBuilder.buildSingBox(hy2Node, [], [], baseSettings, SING_BOX_MIXED_PORT));
    const proxy = config.outbounds.find((outbound) => outbound.tag === "proxy");
  ...
  it("TUIC sing-box: профиль включает heartbeat и zero-rtt policy", () => {
    ...
    const config = parseSingBoxConfig(
      ConfigBuilder.buildSingBox(tuicNode, [], [], { ...baseSettings, routeMode: "global" }, SING_BOX_MIXED_PORT)
    );
    const proxy = config.outbounds.find((outbound) => outbound.tag === "proxy");
  ...
  it("WireGuard sing-box: профиль использует MTU 1408 по умолчанию", () => {
    ...
    const config = parseSingBoxConfig(ConfigBuilder.buildSingBox(wgNode, [], [], baseSettings, SING_BOX_MIXED_PORT));
    const proxy = config.outbounds.find((outbound) => outbound.tag === "proxy");
