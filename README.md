...
- **Telegram Proxy.** Устанавливает собственную Automatic-службу, сохраняет её при обновлении и проверяет локальный порт и реальный upstream-маршрут.
...
Версия 3.5.3 не имеет CA-trusted Authenticode-подписи. Windows может показать SmartScreen; это ограничение не скрывается. Доверие канала обеспечивается Ed25519-манифестом и точными хэшами. Переход с 3.4.3 на ветку 3.5.x выполняется один раз вручную.
